"""
脚本名称: JCM_Full_Crawler_Final.py (V81 - JCM全量究极版)
功能: 
1. 读取本地 dsmz_full_database.csv，筛选所有含 JCM 编号的菌株。
2. 自动断点续传：读取已存在的输出文件，跳过已完成的任务。
3. [一级策略] JCM官网直连：检查是否有 "16S rDNA" 或 "Whole genome" 直连按钮，直接抓取。
4. [二级策略] BacDive解析：提取 Sequence information 中的编号。
5. [三级策略] 顺藤摸瓜：若无序列，提取 "External links" 里的同义编号去 NCBI 反向搜索。
6. 结果保存至 "JCM_seq_Whole" 文件夹及 "JCM数据库全部爬取.csv"。
7. 内置内存保护机制 (每50个重启)。
"""
...
def get_driver():
    ...
    # opts.add_argument("--headless") # 生产环境建议取消注释以静默运行
...
# [新增] 从JCM主页提取"Other collection no."中的同义编号
def get_synonyms_from_jcm_page(driver):
    synonyms = []
    try:
        soup = BeautifulSoup(driver.page_source, "html.parser")
        
        # 查找包含"Other collection no."或"WDCM no."的行
        for text_node in soup.find_all(string=re.compile(r"Other collection no|WDCM no", re.IGNORECASE)):
            parent = text_node.find_parent("tr") or text_node.find_parent("div") or text_node.find_parent("p")
            if parent:
                content = parent.get_text(" ", strip=True)
                # 提取ATCC, CCM, NCIMB等编号
                target_prefixes = ["ATCC", "CCM", "NCIMB", "NCTC", "LMG", "CIP", "NBRC", "IAM", "DSM"]
                for prefix in target_prefixes:
                    matches = re.findall(rf"({prefix})\s*([0-9]+)", content, re.IGNORECASE)
                    for p, n in matches:
                        synonyms.append(f"{p.upper()} {n}")
        
        # 如果没找到，尝试全页面搜索（更宽松的策略）
        if not synonyms:
            body_text = soup.get_text(" ", strip=True)
            target_prefixes = ["ATCC", "CCM", "NCIMB"]
            for prefix in target_prefixes:
                matches = re.findall(rf"({prefix})\s*([0-9]+)", body_text, re.IGNORECASE)
                for p, n in matches:
                    synonyms.append(f"{p.upper()} {n}")
    except: pass
    return list(set(synonyms))

# [核心升级] 顺藤摸瓜：BacDive -> External Links -> 同义词 -> NCBI
...
def main():
    print(f"🚀 启动 JCM 全量爬取任务 (V81 究极生产版)")
    ...
    try:
        for item in tasks_to_do:
            ...
            # 2. 二级策略: 如果直连没搞定，先尝试JCM主页同义词反搜
            if wgs_id == "N/A" or s16_id == "N/A":
                # 2.1 从JCM主页提取ATCC/CCM/NCIMB等编号
                jcm_synonyms = get_synonyms_from_jcm_page(driver)
                if jcm_synonyms:
                    print(f" -> JCM主页同义词:{len(jcm_synonyms)}个 [", end="")
                    for syn in jcm_synonyms:
                        print(f"{syn}..", end="")
                        found = search_ncbi_by_synonym(syn)
                        if found:
                            print(f"✅[{found['id']}]", end="")
                            if found['type'] == "WGS" and wgs_id == "N/A":
                                wgs_id = found["id"]
                                wgs_stat, wgs_file = download_sequence(wgs_id, name, "WGS")
                            elif found['type'] == "16S" and s16_id == "N/A":
                                s16_id = found["id"]
                                s16_stat, s16_file = download_sequence(s16_id, name, "16S")
                            if wgs_id != "N/A" and s16_id != "N/A":
                                break
                    print("]", end="")
                
                # 3. 三级策略: 如果JCM主页同义词也没搞定，去BacDive
                if wgs_id == "N/A" or s16_id == "N/A":
                    ...
                    if bacdive_url:
                        ...
                        # 4. 四级策略: 如果 BacDive 也没序列，启动BacDive同义词反搜
                        if ids["wgs"] == "N/A" and ids["16s"] == "N/A":
                            synonyms = get_synonyms_from_bacdive(driver)
                            if synonyms:
                                print(f" -> BacDive同义词:{len(synonyms)}个 [", end="")
                                for syn in synonyms:
                                    print(f"{syn}..", end="")
                                    found = search_ncbi_by_synonym(syn)
                                    if found:
                    ...
                    print(" [JCM主页同义词已覆盖]", end="")
                    bacdive_url = "N/A (JCM主页同义词)"
            else:
                print(" [JCM全覆盖,跳过后续]", end="")
